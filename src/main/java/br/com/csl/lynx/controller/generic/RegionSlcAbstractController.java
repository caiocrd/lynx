...
public abstract class RegionSlcAbstractController extends AbstractSlcAction {
	...
	public RegionSlcAbstractController() {
	...
	public void toAdequate() {
		try {
			movementHandler.evaluate(slc);
			movementHandler.toAdequate(slc, obs, fotoHandler.getMovFolder());

			clear();
			addFacesInfoMessage("Avaliação concluída com sucesso. SLC enviada para adequação.");
	...
	public void done() {
		try {
			movementHandler.evaluate(slc);
			movementHandler.done(slc, obs, fotoHandler.getMovFolder());

			clear();
			addFacesInfoMessage("Avaliação concluída com sucesso. SLC enviada para finalização!");
		} catch (ServiceException | MovementException | RipException e) {
			addFacesErrorMessage("Erro ao registrar avaliação. Tente novamente!");
		}
	}
