...
@ViewScoped
@ManagedBean
public class AtendenteSlpController extends UsersAbstractController {

	private static final long serialVersionUID = 1L;

	public AtendenteSlpController() {
		clear();
	}

	@PostConstruct
	public void init() {
		usersRolesDataModel.newRestraints("r.name", Permission.ATENDENTE_SLP.name());
	}
	...
	public void remove(Usuario user) {
		...
		u.getRoles().remove(roleService.find(Restrictions.eq("name", Permission.ATENDENTE_SLP.name())));
	...
	public void saveUser() {
			...
			if (u != null) {
				if (usersRolesService.count(
							Restrictions.and(
									...
									Restrictions.or(
											Restrictions.eq("r.name", Permission.ATENDENTE_SLP.name()), 
											Restrictions.eq("r.name", Permission.CALLCENTER_SLP.name())))) == 0) {
					
					u.getRoles().add(roleService.find("name", Permission.ATENDENTE_SLP.name()));
		...
		if (user.getId() == null) {
			...
			user.getRoles().add(roleService.find("name", Permission.ATENDENTE_SLP.name()));
		}
