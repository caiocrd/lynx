...
@ViewScoped
@ManagedBean
public class AtendenteController extends UsersAbstractController {
	...
	@PostConstruct
	public void init() {
		usersRolesDataModel.newRestraints("r.name", Permission.ATENDENTE.name());
	}
	...
	public void saveUser() {
				...
				if (usersRolesService.count(
							Restrictions.and(
									...
									Restrictions.or(
											...
											Restrictions.eq("r.name", Permission.CALLCENTER.name())))) == 0) {
					
					u.getRoles().add(roleService.find("name", Permission.ATENDENTE.name()));
		...
		if (user.getId() == null) {
			...
			user.getRoles().add(roleService.find("name", Permission.ATENDENTE.name()));
		}
