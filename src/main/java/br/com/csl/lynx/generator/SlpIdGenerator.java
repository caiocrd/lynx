...
public class SlpIdGenerator extends IdentityGenerator implements IdentifierGenerator {
	...
	public synchronized Serializable generate(SessionImplementor session, Object obj) throws HibernateException {
		if ((((PersistentEntity) obj).getId()) == null) {
	        ...
	        try {
	            ...
	            PreparedStatement ps = connection.prepareStatement("SELECT MAX(id) AS maxId FROM slp WHERE id >= "+ prefix +" AND id <= "+ maxId);
	            ...
	            if (rs.first()) {
	            	...
	            	if (id != 0) {
	            		if (id < prefix + 9999) {	
	            			id = id + 1;
	            		} else throw new HibernateException("O limite de SLP's por mês foi atingido!");  
	            ...
	            throw new HibernateException("Não foi possível gerar o código para a SLP.");
	        }
