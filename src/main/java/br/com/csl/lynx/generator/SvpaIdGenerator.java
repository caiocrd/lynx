...
public class SvpaIdGenerator extends IdentityGenerator implements IdentifierGenerator {
	...
	public synchronized Serializable generate(SessionImplementor session, Object obj) throws HibernateException {
		if ((((PersistentEntity) obj).getId()) == null) {
	        ...
	        try {
	            ...
	            PreparedStatement ps = connection.prepareStatement("SELECT MAX(id) AS maxId FROM svpa WHERE id >= "+ prefix +" AND id <= "+ maxId);
