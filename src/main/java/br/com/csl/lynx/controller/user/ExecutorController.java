...
@ViewScoped
@ManagedBean
public class ExecutorController extends UsersAbstractController {
	...
	@PostConstruct
	public void init() {
		usersRolesDataModel.newRestraints("r.name", Permission.EXECUTOR.name());
	}
	...
	public void remove(Usuario user) {
		...
		u.getRoles().remove(roleService.find(Restrictions.eq("name", Permission.EXECUTOR.name())));
	...
	public void saveUser() {
			...
			if (u != null) {
				if (usersRolesService.count(
							Restrictions.and(
									...
									Restrictions.or(
											Restrictions.eq("r.name", Permission.EXECUTOR.name()), 
											Restrictions.eq("r.name", Permission.PRESTADORA.name())))) == 0) {
					
					u.getRoles().add(roleService.find("name", Permission.EXECUTOR.name()));
		...
		if (user.getId() == null) {
			...
			user.getRoles().add(roleService.find("name", Permission.EXECUTOR.name()));
		}
