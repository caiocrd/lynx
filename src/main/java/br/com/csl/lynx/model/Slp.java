...
@Entity
@Table(name = "slp")
public class Slp implements DomainObject {
	...
	private List<MovimentacaoSlp> movimentacoes;
	private List<QtdServicoSlp> qtdServicos;
	private List<OcorrenciaSlp> ocorrencias;
	...
	@Id
	@GeneratedValue(strategy = GenerationType.IDENTITY, generator = "SlpIdGenerator")
	@GenericGenerator(name = "SlpIdGenerator", strategy = "br.com.csl.lynx.generator.SlpIdGenerator")
	public Long getId() {
		return id;
	}
	...
	@BatchSize(size = 500)
	@OrderBy("id ASC")
	@OneToMany(mappedBy = "slp", cascade = CascadeType.ALL, fetch=FetchType.EAGER)
	public List<MovimentacaoSlp> getMovimentacoes() {
		return movimentacoes;
	}

	public void setMovimentacoes(List<MovimentacaoSlp> movimentacoes) {
	...
	@JoinTable(name = "slp_qtdservico")
	public List<QtdServicoSlp> getQtdServicos() {
		return qtdServicos;
	}

	public void setQtdServicos(List<QtdServicoSlp> qtdServicos) {
	...
	@OrderBy("id ASC")
	@OneToMany(mappedBy = "slp", cascade = CascadeType.ALL, fetch = FetchType.EAGER)
	public List<OcorrenciaSlp> getOcorrencias() {
		return ocorrencias;
	}
	
	public void setOcorrencias(List<OcorrenciaSlp> ocorrencias) {
