...
@ViewScoped
@ManagedBean
public class ExecutorSlcController extends UsersAbstractController {

	private static final long serialVersionUID = 1L;

	public ExecutorSlcController() {
		clear();
	}
	
	@PostConstruct
	public void init() {
		usersRolesDataModel.newRestraints("r.name", Permission.EXECUTOR_SLC.name());
	}
	...
	public void remove(Usuario user) {
		...
		u.getRoles().remove(roleService.find(Restrictions.eq("name", Permission.EXECUTOR_SLC.name())));
	...
	public void saveUser() {
			...
			if (u != null) {
				if (usersRolesService.count(
							Restrictions.and(
									...
									Restrictions.or(
											Restrictions.eq("r.name", Permission.EXECUTOR_SLC.name()), 
											Restrictions.eq("r.name", Permission.PRESTADORA_SLC.name())))) == 0) {
					
					u.getRoles().add(roleService.find("name", Permission.EXECUTOR_SLC.name()));
		...
		if (user.getId() == null) {
			...
			user.getRoles().add(roleService.find("name", Permission.EXECUTOR_SLC.name()));
		}
