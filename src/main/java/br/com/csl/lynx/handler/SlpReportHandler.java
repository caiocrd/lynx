...
@Component
public class SlpReportHandler implements Serializable {

	private static final long serialVersionUID = 1L;

	public synchronized StreamedContent getSlpReport(List<Slp> slps) throws JRException, IOException {
		Calendar now = Calendar.getInstance(TimeZone.getTimeZone("GMT-03:00"));
		
		JRBeanCollectionDataSource beanCollectionDataSource = new JRBeanCollectionDataSource(slps);
		
		String reportPath = FacesContext.getCurrentInstance().getExternalContext().getRealPath("/resources/jasper/receiveSlp.jasper");
	    ...
	    return new DefaultStreamedContent(stream, "application/pdf", "slps-"+  sdf.format(now.getTime())  +".pdf"); 
	}	
	
	public synchronized StreamedContent getSimpleSlpReport(List<Slp> slps) throws JRException, IOException {
		Calendar now = Calendar.getInstance(TimeZone.getTimeZone("GMT-03:00"));
		
		JRBeanCollectionDataSource beanCollectionDataSource = new JRBeanCollectionDataSource(slps);
		
		String reportPath = FacesContext.getCurrentInstance().getExternalContext().getRealPath("/resources/jasper/slp.jasper");
