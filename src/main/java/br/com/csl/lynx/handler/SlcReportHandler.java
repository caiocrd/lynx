...
@Component
public class SlcReportHandler implements Serializable {

	private static final long serialVersionUID = 1L;

	public synchronized StreamedContent getSlcReport(List<Slc> slcs) throws JRException, IOException {
		Calendar now = Calendar.getInstance(TimeZone.getTimeZone("GMT-03:00"));
		
		JRBeanCollectionDataSource beanCollectionDataSource = new JRBeanCollectionDataSource(slcs);
		
		String reportPath = FacesContext.getCurrentInstance().getExternalContext().getRealPath("/resources/jasper/receiveSlc.jasper");
	    ...
	    return new DefaultStreamedContent(stream, "application/pdf", "slcs-"+  sdf.format(now.getTime())  +".pdf"); 
	}	
	
	public synchronized StreamedContent getSimpleSlcReport(List<Slc> slcs) throws JRException, IOException {
		Calendar now = Calendar.getInstance(TimeZone.getTimeZone("GMT-03:00"));
		
		JRBeanCollectionDataSource beanCollectionDataSource = new JRBeanCollectionDataSource(slcs);
		
		String reportPath = FacesContext.getCurrentInstance().getExternalContext().getRealPath("/resources/jasper/slc.jasper");
