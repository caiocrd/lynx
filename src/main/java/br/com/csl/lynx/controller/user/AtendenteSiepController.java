...
@ViewScoped
@ManagedBean
public class AtendenteSiepController extends UsersAbstractController {

	private static final long serialVersionUID = 1L;

	public AtendenteSiepController() {
		clear();
	}

	@PostConstruct
	public void init() {
		usersRolesDataModel.newRestraints("r.name", Permission.ATENDENTE_SIEP.name());
	}
	...
	public void remove(Usuario user) {
		...
		u.getRoles().remove(roleService.find(Restrictions.eq("name", Permission.ATENDENTE_SIEP.name())));
	...
	public void saveUser() {
			...
			if (u != null) {
				if (usersRolesService.count(
							Restrictions.and(
									...
									Restrictions.or(
											Restrictions.eq("r.name", Permission.ATENDENTE_SIEP.name()), 
											Restrictions.eq("r.name", Permission.CALLCENTER_SIEP.name())))) == 0) {
					
					u.getRoles().add(roleService.find("name", Permission.ATENDENTE_SIEP.name()));
		...
		if (user.getId() == null) {
			...
			user.getRoles().add(roleService.find("name", Permission.ATENDENTE_SIEP.name()));
		}
