...
@ManagedBean
@ViewScoped
public class ReportController extends AbstractRipInfo {
	...
	public void createReport() {
		...
		if (status != null) {
			ripFilters.add(Restrictions.eq("status", status));
		...
		if (!posteBto.isEmpty()) {
			ripFilters.add(Restrictions.eq("p.numero", posteBto));
		...
		if (!zonas.isEmpty()) {
			for (String zona : zonas) {
				if (zonaDisabled) {
					ripFilters.add(Restrictions.eq("bairro.zona", Zona.valueOf(zona)));
				} else {
		...
		if (!solicitante.isEmpty()) {
			ripFilters.add(Restrictions.like("solicitante", solicitante, MatchMode.ANYWHERE));
	...
	public void populateReport() {
		...
		report.setTotal(rips.size());
		
		for (Rip rip : rips) {
			List<Movement> movements = new ArrayList<>();
			
			RipReport ripReport = new RipReport(rip.getId(), rip.getPrevisao().getTime());
			ripReport.setOccurences(rip.getOcorrencias().size());

			if (ripReport.getOccurrences() > 0) {							
				ripReport.setLastOccurrence(CalendarUtil.dateToString(rip.getOcorrencias().get(rip.getOcorrencias().size() - 1).getData().getTime()));
			}
			
			Boolean expired = false;
			
			for (Movimentacao movimentacao : rip.getMovimentacoes()) {
				
				ripReport.setDates(movimentacao);
				
				if (movimentacao.getMovimento().equals(Movement.EXECUTE))
					if (rip.getPrevisao().before(movimentacao.getData()))
						expired = true;
				...
				for (StatusReport statusReport : statusReports)
					if (rip.getStatus().equals(statusReport.getStatus()))
						statusReport.increment(movimentacao.getMovimento());
			
				for (ProblemReport problemReport : problemReports)
					if (rip.getTipoReclamacao().equals(problemReport.getProblem()))
						problemReport.increment(movimentacao.getMovimento());	
			}
			
			if (!movements.contains(Movement.EXECUTE) && rip.getStatus() != RipStatus.CANCELED && rip.getPrevisao().before(CalendarUtil.getToday()))
				expired = true;
			
			if (expired)
				report.incrementExpired();
			
			report.increment(rip.getStatus());
			
			for (ProblemReport problemReport : problemReports) {
				if (rip.getTipoReclamacao().equals(problemReport.getProblem())) {
					problemReport.incrementTotal();
					problemReport.increment(rip.getStatus());
					if (expired)
	...
	public List<String> loadSolicitantes(String query) {
		...
		if (query != null && !query.isEmpty()) {
			...
			if (words.length > 0) {
				...
				for (Rip aux : ripService.list(0, 5, criterions, null)) {
	...
	public StreamedContent print() {
		...
		try {
			 pdf = printRipHandler.printReport(report);
