...
public abstract class RegionSvpaAbstractController extends AbstractSvpaAction {
	...
	public RegionSvpaAbstractController() {
	...
	public void toAdequate() {
		try {
			movementHandler.evaluate(svpa);
			movementHandler.toAdequate(svpa, obs, fotoHandler.getMovFolder());

			clear();
			addFacesInfoMessage("Avaliação concluída com sucesso. SVPA enviada para adequação.");
	...
	public void done() {
		try {
			movementHandler.evaluate(svpa);
			movementHandler.done(svpa, obs, fotoHandler.getMovFolder());

			clear();
			addFacesInfoMessage("Avaliação concluída com sucesso. SVPA enviada para finalização!");
		} catch (ServiceException | MovementException | RipException e) {
			addFacesErrorMessage("Erro ao registrar avaliação. Tente novamente!");
		}
	}
