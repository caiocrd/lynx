...
@Component
public class SvpaReportHandler implements Serializable {

	private static final long serialVersionUID = 1L;

	public synchronized StreamedContent getSvpaReport(List<Svpa> svpas) throws JRException, IOException {
		Calendar now = Calendar.getInstance(TimeZone.getTimeZone("GMT-03:00"));
		
		JRBeanCollectionDataSource beanCollectionDataSource = new JRBeanCollectionDataSource(svpas);
		
		String reportPath = FacesContext.getCurrentInstance().getExternalContext().getRealPath("/resources/jasper/receiveSvpa.jasper");
	    ...
	    return new DefaultStreamedContent(stream, "application/pdf", "svpas-"+  sdf.format(now.getTime())  +".pdf"); 
	}	
	
	public synchronized StreamedContent getSimpleSvpaReport(List<Svpa> svpas) throws JRException, IOException {
		Calendar now = Calendar.getInstance(TimeZone.getTimeZone("GMT-03:00"));
		
		JRBeanCollectionDataSource beanCollectionDataSource = new JRBeanCollectionDataSource(svpas);
		
		String reportPath = FacesContext.getCurrentInstance().getExternalContext().getRealPath("/resources/jasper/svpa.jasper");
